...
export const StringForestView = ({
	forest, makePath, selectedPath, onItemClick, onItemDoubleclick, ...props
}: Props) => {
	...
	let innerProps: TreeViewWithCreationProps<string, string> = {
		...
		onBranchClick: !onItemClick ? undefined : (_, path) => onItemClick(makePath(treePathToValues(forest, path), true), true),
		onBranchDoubleclick: !onItemDoubleclick ? undefined : (_, path) => onItemDoubleclick(makePath(treePathToValues(forest, path), true), true),
