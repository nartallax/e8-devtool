...
export async function projectToTypescript(project: Project, config: Config): Promise<string> {
	...
	const bindSetsEnumName = config.ts.inputBindSetEnumName
	const bindSetsNamespaceName = config.ts.inputBindsNamespaceName
...
${convertClassPathsToImports(classPaths, config.ts.path).join("\n")}
...
export type EngineInstance = Engine<${modelEnumName}, ${bindSetMappingName}, ${config.ts.particleEnumName}>
