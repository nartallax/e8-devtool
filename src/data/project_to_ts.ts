...
/*
	const [inputBindForest, inputBindMap, config, particlesForest, particlesMap, modelsForest, modelsMap] = await Promise.all([
		...
		actions.dirs.models.getForest(),
		actions.dirs.models.getAllItemsAsMap()
	])

	const classPaths = await gatherEntityClassPaths(actions.resolveProjectPath(config.entityClassesDirectoryPath))

	const modelEntries = getAllProjectModelsWithFolders(modelsForest, modelsMap)
		.map(([pathParts]) => pathParts)

	const sortedBinds = getSortedProjectBinds(inputBindForest, inputBindMap)
	const bindSetNames = sortedBinds.map(([,path]) => path)
	const particleNames = mappedForestToArrayWithPath(particlesForest, particlesMap).map(([, path]) => path)
	const modelEnumName = config.ts.entityEnumName
	const bindsEnumName = config.ts.inputBindsEnumName
	const loaderVariableName = config.ts.loaderVariableName
...
${convertClassPathsToImports(classPaths, actions.resolveProjectPath(config.ts.path)).join("\n")}
...
export type EngineInstance = Engine<${modelEnumName}, ${bindsEnumName}, ${config.ts.particleEnumName}>
...
${getConstEnumCode("", config.ts.particleEnumName, "All particle presets available in the resource pack", particleNames)}
`
}
