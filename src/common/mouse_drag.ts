...
export function pointerEventsToOffsetCoords(e: MouseEvent | TouchEvent, target: unknown = null): XY | null {
	target ??= e.target
...
export function addMouseDragHandler(params: MouseDragHandlerParams): MouseDragHandlers {
	...
	function stopMoving(e: MouseEvent | TouchEvent | null, isUp: boolean, canTriggerClick = true): void {
	...
	} else if(params.element instanceof SVGElement){
		setDragTargetState(params.element, true)
		params.element.addEventListener("mousedown", onDown)
		params.element.addEventListener("touchstart", onDown)
	} else {
	...
	const clickPreventingHandler = (e: Event) => {
		if(!targetIsRight(e)){
			return
		}
		if(!isClickingNow){
	...
	if(isClickPreventionEnabled){
		params.element.addEventListener("click", clickPreventingHandler, {capture: true})
	}

	const detachDragHandlers = () => {
		if(params.element instanceof HTMLElement){
		...
		if(isClickPreventionEnabled){
			params.element.removeEventListener("click", clickPreventingHandler, {capture: true})
		}
	}

	return {
		detach: detachDragHandlers,
		cancelCurrent: () => stopMoving(null, false)
	}
}

const dataAttrName = "data-is-drag-target"

function setDragTargetState(el: HTMLElement | SVGElement, isDragTarget: boolean): void {
	if(!isDragTarget){
...
function findDragTarget(e: Event): HTMLElement | SVGElement | Window {
	let el = e.target
	while(el){
